    ...
    def number_input(self):
        ...
        while not valid_input:  
            try: 
                ...
                numbers = list(map(float,numb_input.split( ))) #take up to two numbers turn them into intergers, split them at the comma, and add to a list
                valid_input = True 
                return numbers
            except:
...
if __name__ == '_main_':
    unittest.main() 
# test is created so that we don't have to check each time
...
if len(numbers) == 1:# when input is 1 number
    ...
    print calculator.xy_function(numbers[0],numbers[1]) #run function two on first and second index which gives add, subtract, multiply, divide and exponential
